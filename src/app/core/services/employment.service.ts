...
export class EmploymentService {
    ...
    public getHardListById(
        ...
        const url = `${this.baseUrl}${this.apiUrl}/hard/id/${id}?sort=${sort}&order=${order}&size=${pageSize}&page=${pageNumber}`;
        const result = this.http.get<PageList<any>>(url);
        return result;
    }

    public getHardListBySi(
        si: string,
        sort: string,
        order: string,
        pageNumber: number,
        ...
        const url = `${this.baseUrl}${this.apiUrl}/hard/si/${si}?sort=${sort}&order=${order}&size=${pageSize}&page=${pageNumber}`;
        const result = this.http.get<any>(url);
        return result;
    }
}
